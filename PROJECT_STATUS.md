...
[`artifacts/local_ai/stage_b/`](artifacts/local_ai/stage_b/), regenerated by
`stage-b-candidate-generator-v4`: 3,600 provisional rows across 600 source
groups and 32 template families. Scope counts remain 1,800 supported play,
1,260 supported semantic-unknown, 300 deterministic-only, and 240 safety-only.
Language counts are zh-Hant 1,380, zh-Hans 288, mixed 1,140, and en 792. The
supported-language reserve is zh-Hant 1,200 (720 play / 480 unknown) and mixed
960 (540 play / 420 unknown). Slot counts are artist present 1,080 / absent
720 and album present 1,080 / absent 720. The play-group slot matrix is frozen
as zh-Hant 48/24/24/24, zh-Hans 9/5/5/5, mixed 36/18/18/18, and en
27/13/13/13 for both/artist-only/album-only/neither.

The v4 entity catalog is deterministic, local, synthetic, provider-ID free,
and Spotify/network independent. Chinese artist morphology is zh-Hant
69 band-suffix / 92 solo-style / 69 group-no-suffix (30.0% band suffix) and
zh-Hans 15 / 20 / 13 (31.25% band suffix). English artist starts-with-The is
13/132 and mixed is 19/190. English artist/track/album word-count buckets are
respectively 27/40/39/26, 32/38/45/17, and 34/32/43/23 for 1/2/3/4+;
mixed is 38/57/57/38, 47/55/64/24, and 48/48/60/34. Digit-bearing entity
counts (any field) are zh-Hant 58, zh-Hans 12, mixed 70, and en 49; safe
...
mixed rows without an ASCII English letter 0, and English rows with CJK 0.
Stage A leakage, exact duplicates, same-group near duplicates, cross-group near
duplicates, and cross-group exact duplicates are all zero. Every row remains
`pending_independent_review`; the deterministic quality spot-check fixture
covers zh-Hant/zh-Hans/mixed/English play, zh-Hant/mixed semantic unknown,
deterministic-only, safety-only, and ASR/homophone examples. The offline
production-alignment audit uses the real CommandParser and
SemanticRetryEligibilityGate: supported play/unknown are 1,800/1,260 eligible
rows, while deterministic-only/safety-only are 300/240 blocked rows; scope
...
This is **not** the final Stage B corpus: independent review is still pending,
no final split is assigned, held-out data is not sealed, the 3,000-row corpus
is not accepted, and training, fine-tuning, inference, model compute, RX 9070
